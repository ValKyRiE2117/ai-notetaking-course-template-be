...
func (n *noteRepository) GetByNotebookIds(ctx context.Context, ids []uuid.UUID) ([]*entity.Note, error) {
	...
	rows, err := n.db.Query(
		ctx,
		fmt.Sprintf(`SELECT id, title, content, notebook_id, created_at, updated_at, deleted_at, is_deleted FROM note WHERE notebook_id IN (%s) AND is_deleted = false`, idSqlFormat),
	)
	if err != nil {
		return nil, err
	}
